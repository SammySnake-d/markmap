# markmap-mcp-service

一个用于从 Markdown 内容生成交互式思维导图 HTML 文件的 MCP (Model Context Protocol) 服务。

## 概述

本包提供一个 MCP 服务器，暴露 `generate_mindmap` 工具。该工具接收 Markdown 内容，生成包含交互式思维导图可视化的独立 HTML 文件。

## 功能特性

- 🎯 **MCP 兼容** - 支持任何 MCP 兼容客户端（Kiro、Claude Desktop 等）
- 📝 **Markdown 输入** - 支持标准 Markdown 及增强的备注语法
- 🎨 **多种主题** - 5 种内置配色方案 + 明暗模式
- 📤 **独立输出** - 生成的 HTML 文件可离线使用
- ✏️ **编辑模式** - 可选的浏览器内编辑支持
- 🔍 **搜索功能** - 生成文件内置搜索功能
- 📱 **移动端支持** - 触摸手势缩放和平移

## 安装

```bash
# 使用 pnpm（推荐）
pnpm add markmap-mcp-service

# 使用 npm
npm install markmap-mcp-service

# 使用 yarn
yarn add markmap-mcp-service
```

## 快速开始

### 1. 配置 MCP 客户端

将服务添加到 MCP 客户端配置中：

**Kiro/Claude Desktop 配置：**

```json
{
  "mcpServers": {
    "markmap": {
      "command": "node",
      "args": ["path/to/node_modules/markmap-mcp-service/dist/index.js"]
    }
  }
}
```

**全局安装后：**

```json
{
  "mcpServers": {
    "markmap": {
      "command": "markmap-mcp"
    }
  }
}
```

### 2. 使用工具

配置完成后，即可使用 `generate_mindmap` 工具从 Markdown 内容创建思维导图 HTML 文件。

### 3. 自定义默认配置

通过环境变量配置默认值：

```json
{
  "mcpServers": {
    "markmap": {
      "command": "node",
      "args": ["path/to/markmap-mcp-service/dist/index.js"],
      "env": {
        "MARKMAP_NOTE_SEPARATOR": "|",
        "MARKMAP_NOTE_BLOCK": "#",
        "MARKMAP_COLOR_SCHEME": "ocean",
        "MARKMAP_THEME": "dark"
      }
    }
  }
}
```

## 环境变量配置

通过环境变量可以自定义 MCP 服务的默认行为。配置后，AI 在调用工具时会看到当前配置的分隔符信息。

### 分隔符配置

| 环境变量 | 说明 | 默认值 | 示例 |
|----------|------|--------|------|
| `MARKMAP_NOTE_SEPARATOR` | 单行备注分隔符 | `:` | `- 节点: 备注` |
| `MARKMAP_NOTE_BLOCK` | 多行备注块标记 | `>` | `> 详细备注` |
| `MARKMAP_ESCAPE` | 转义字符 | `\\` | `\:` 转义冒号 |

**分隔符作用说明：**

- **单行备注分隔符** (`MARKMAP_NOTE_SEPARATOR`)：用于在节点内容后添加简短备注
  ```markdown
  - 节点内容: 这是单行备注
  ```
  如果设置为 `|`，则变为：
  ```markdown
  - 节点内容| 这是单行备注
  ```

- **多行备注块标记** (`MARKMAP_NOTE_BLOCK`)：用于添加多行详细备注
  ```markdown
  - 节点内容
    > 详细备注第一行
    > 详细备注第二行
  ```
  如果设置为 `#`，则变为：
  ```markdown
  - 节点内容
    # 详细备注第一行
    # 详细备注第二行
  ```

- **转义字符** (`MARKMAP_ESCAPE`)：用于在内容中使用分隔符字符本身
  ```markdown
  - 时间\: 10:00  # 冒号被转义，不会被识别为备注分隔符
  ```

### 外观配置

| 环境变量 | 说明 | 默认值 | 可选值 |
|----------|------|--------|--------|
| `MARKMAP_COLOR_SCHEME` | 默认配色方案 | `default` | `default`, `ocean`, `forest`, `sunset`, `monochrome` |
| `MARKMAP_THEME` | 默认主题 | `light` | `light`, `dark` |

### 其他配置

| 环境变量 | 说明 | 默认值 |
|----------|------|--------|
| `MARKMAP_CDN_BASE` | CDN 基础路径 | `https://cdn.jsdelivr.net/npm` |

### 动态工具描述

MCP 服务会根据环境变量配置动态生成工具描述，AI 在调用 `tools/list` 时会看到当前配置的分隔符信息，例如：

```
当前配置的分隔符：
- 单行备注分隔符: "|" (例: "- 节点内容| 这是备注")
- 多行备注块标记: "#" (例: "# 详细备注内容")
```

这样 AI 就能根据实际配置生成正确格式的 Markdown 内容。

> **注意**：工具调用时传入的 `separators` 参数会覆盖环境变量配置。

## 工具参考

### generate_mindmap

从 Markdown 生成交互式思维导图 HTML 文件。

#### 输入参数

| 参数 | 类型 | 必需 | 默认值 | 说明 |
|------|------|------|--------|------|
| `markdown` | string | ✅ 是 | - | Markdown 内容 |
| `outputPath` | string | ✅ 是 | - | 输出 HTML 文件路径 |
| `title` | string | 否 | "Mindmap" | 页面标题 |
| `colorScheme` | string | 否 | "default" | 配色方案名称 |
| `enableEdit` | boolean | 否 | `true` | 启用编辑模式 |
| `theme` | string | 否 | "light" | 主题模式 |
| `cdnBase` | string | 否 | "https://cdn.jsdelivr.net/npm" | CDN 基础路径 |
| `separators` | object | 否 | 见下表 | 分隔符配置 |

#### 分隔符配置 (separators)

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `note` | string | ":" | 单行备注分隔符 |
| `noteBlock` | string | ">" | 多行备注块标记 |
| `escape` | string | "\\\\" | 转义字符 |

#### 配色方案

| 名称 | 说明 |
|------|------|
| `default` | 默认蓝紫渐变 |
| `ocean` | 蓝绿海洋色调 |
| `forest` | 绿色自然色调 |
| `sunset` | 暖橙红色调 |
| `monochrome` | 灰度 |

#### 主题选项

| 值 | 说明 |
|----|------|
| `light` | 浅色背景（默认） |
| `dark` | 深色背景 |

#### 输出格式

**成功响应：**
```json
{
  "success": true,
  "filePath": "./output/mindmap.html",
  "message": "Mindmap generated successfully at ./output/mindmap.html"
}
```

**错误响应：**
```json
{
  "success": false,
  "message": "Failed to generate mindmap: [错误详情]",
  "error": "[错误详情]"
}
```

## Markdown 格式

服务支持带备注语法的增强 Markdown：

### 基本结构

```markdown
# 主题

- 节点 1
  - 子节点 1.1
  - 子节点 1.2
- 节点 2
  - 子节点 2.1
```

### 单行备注（使用 `:`）

在冒号后添加简短备注：

```markdown
- 节点内容: 这是单行备注
- 另一个节点: 简短描述
```

### 详细备注（使用 `>`）

使用引用块添加多行详细备注：

```markdown
- 节点内容
  > 这是详细备注
  > 可以跨多行
  > 支持 **Markdown** 格式
```

### 混合备注

同时使用单行和详细备注：

```markdown
- 节点内容: 简短单行备注
  > 详细说明
  > 更多信息
```

### 转义字符

使用反斜杠转义特殊字符：

```markdown
- 内容中包含 \: 冒号的节点
- 另一个节点: 备注中有 \: 转义冒号
```

## 示例

### 基本示例

```json
{
  "markdown": "# 我的项目\n\n- 功能 1\n  - 子功能 1.1\n  - 子功能 1.2\n- 功能 2",
  "outputPath": "./output/project.html"
}
```

### 完整参数示例

```json
{
  "markdown": "# AI 学习路径\n\n- 基础: 基础知识\n  - 数学\n    - 线性代数\n    - 统计学\n  - 编程\n    - Python\n    - NumPy\n- 机器学习\n  > 核心 ML 概念\n  > 监督学习和无监督学习\n  - 回归\n  - 分类",
  "outputPath": "./output/ai-learning.html",
  "title": "AI 学习路径",
  "colorScheme": "ocean",
  "enableEdit": true,
  "theme": "light"
}
```

### 自定义分隔符示例

如果你的 Markdown 使用不同的分隔符格式，可以自定义：

```json
{
  "markdown": "# 项目计划\n\n- 阶段一 | 这是备注\n  - 任务 1\n  - 任务 2\n- 阶段二\n  # 详细说明\n  # 多行备注",
  "outputPath": "./output/custom.html",
  "separators": {
    "note": "|",
    "noteBlock": "#"
  }
}
```

### 自定义 CDN 路径示例

```json
{
  "markdown": "# 示例\n- 节点 1\n- 节点 2",
  "outputPath": "./output/cdn.html",
  "cdnBase": "https://unpkg.com"
}
```

### 只读示例

```json
{
  "markdown": "# 文档\n\n- 第一章\n- 第二章",
  "outputPath": "./docs/overview.html",
  "title": "文档概览",
  "enableEdit": false
}
```

## 生成的 HTML 功能

生成的 HTML 文件包含以下功能：

| 功能 | 说明 |
|------|------|
| 🔍 **搜索** | 按内容或备注查找节点（Cmd/Ctrl + F） |
| 📝 **编辑模式** | 在浏览器中直接编辑备注 |
| 🎨 **颜色主题** | 切换配色方案 |
| 📤 **导出** | 导出为 PNG、JPG、SVG 或 Markdown |
| 📱 **移动端支持** | 触摸手势缩放和平移 |
| 💾 **自动保存** | 更改保存到 localStorage |
| ⌨️ **键盘快捷键** | 撤销（Cmd/Ctrl+Z）、重做（Cmd/Ctrl+Shift+Z） |
| 🖱️ **画布控制** | 滚轮缩放、空格+拖拽平移 |
| 🖱️ **右键菜单** | 复制节点/子树为 Markdown |

> **注意**：MCP 服务默认使用本地打包模式，生成的 HTML 文件较大（约 800KB+），但包含完整功能且可离线使用。

## 编程方式使用

也可以通过编程方式使用服务：

```typescript
import { generateStandaloneHTML } from 'markmap-html-generator';
import * as fs from 'fs';

const markdown = `# 我的思维导图
- 主题 1: 备注
- 主题 2`;

const html = generateStandaloneHTML(markdown, {
  title: '我的思维导图',
  colorScheme: 'ocean',
  enableEdit: true,
  theme: 'light',
});

fs.writeFileSync('./output/mindmap.html', html);
```

## 错误处理

服务处理各种错误情况：

| 错误类型 | 说明 | 响应 |
|----------|------|------|
| 缺少必需字段 | 未提供 `markdown` 或 `outputPath` | 验证错误 |
| 无效配色方案 | 未知的配色方案名称 | 验证错误 |
| 无效主题 | 主题不是 "light" 或 "dark" | 验证错误 |
| 文件写入错误 | 无法写入输出路径 | 文件系统错误 |
| 解析错误 | 无效的 Markdown 格式 | 解析错误 |

## 开发

```bash
# 安装依赖
pnpm install

# 构建
pnpm build

# 运行测试
pnpm test

# 启动服务器（用于测试）
pnpm start
```

## 系统要求

- Node.js >= 18
- markmap-html-generator（对等依赖）

## 许可证

MIT

## 相关包

- [markmap-html-generator](../markmap-html-generator) - HTML 生成库
- [markmap-lib](../markmap-lib) - Markdown 解析库
- [markmap-view](../markmap-view) - 可视化库
